...
func TestFunctional(t *testing.T) {
	...
	// Failure scenario-create acct withoout uuid
	a, e = AccountCreateWithoutId()
	if e == nil {
		t.Fatal(errors.New("test failed AccountCreateWithoutId"))
	}
	// Fetch an invalid id
	if e = AccountFetchWrongId("invalidid"); e == nil {
		t.Fatal(errors.New("test failed AccountFetchWrongId"))
	}
	// delete an account withut version-failure scenario
	if e = AccountDeleteNoVersion(ID, strconv.Itoa(int(v))); e == nil {
		t.Fatal(errors.New("test failed AccountDeleteNoVersion"))
	}
...
func AccountCreateWithoutId() (id account.Data, e error) {
	add := setup()
	c := account.NewAcctController(add)
	acct := &account.AccountData{
		ID:             "",
...
func AccountFetchWrongId(id string) error {
	add := setup()
	c := account.NewAcctController(add)
	fetchedAcc, e := c.FetchAccount(id)
	if e != nil {
...
func AccountDeleteNoVersion(id string, version string) error {
	add := setup()
	c := account.NewAcctController(add)
	if e := c.DeleteAccount(id, version); e != nil {
		return e
